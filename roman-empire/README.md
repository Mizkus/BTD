...
- Авторизация через JWT: страницы /login и /register в интерфейсе, токен хранится в `localStorage`, автоматически добавляется в `Authorization` заголовки.
- Статистика доступна только роли `admin`. При старте сервера создаётся пользователь `admin@example.com` с паролем `admin123`.
...
- `/auth/register` — регистрация пользователя с ролью `user`.
- `/auth/token` — получение JWT (используется формой логина в React).
- `/auth/me` — текущий пользователь.
- Интерфейс автоматически перенаправляет неавторизованных пользователей на `/login`, а вкладка «Статистика» видна только администратору.
